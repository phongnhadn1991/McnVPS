...
for tg_controller in "${MENU_DIR}"/controllers/telegram/*.sh; do
    if [ -f "$tg_controller" ]; then
        # shellcheck source=/var/mcnvps/server-manager/controllers/telegram/*.sh
        source "$tg_controller"
    fi
done
