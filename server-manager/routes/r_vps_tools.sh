...
for vps_controller in "${MENU_DIR}"/controllers/vps/*.sh; do
    if [ -f "$vps_controller" ]; then
        # shellcheck source=/var/mcnvps/server-manager/controllers/vps/*.sh
        source "$vps_controller"
    fi
done

for tools_controller in "${MENU_DIR}"/controllers/tools/*.sh; do
    if [ -f "$tools_controller" ]; then
        # shellcheck source=/var/mcnvps/server-manager/controllers/tools/*.sh
        source "$tools_controller"
    fi
done

for tunnel_controller in "${MENU_DIR}"/controllers/tunnel/*.sh; do
    if [ -f "$tunnel_controller" ]; then
        # shellcheck source=/var/mcnvps/server-manager/controllers/tunnel/*.sh
        source "$tunnel_controller"
    fi
done
...
vps_tools_menu() {
    ...
    while true; do
        ...
        echo "${BLUE}5. Nen anh (Image Compression)${NC}"
        echo "${BLUE}6. Antivirus (ClamAV/ImunifyAV)${NC}"
        echo "${BLUE}7. Cloudflare Tunnel${NC}"
        echo "${RED}----------------------------------------${NC}"
