...
_enable_wp_seo_plugin() {
    ...
    if [[ -z "$plugin_name" || -z "$conf_file" || -z "$conf_remove" ]]; then
        msg "$ICON_EXIT enable_wp_seo_plugin error: Thieu tham so bat buoc!"
        exit 1
    fi

    run_prompt_or_exit prompt_select_website domain "wp_seo_plugin_menu" "$WEB_DATA_DIR" 'd' 'wordpress'
...
enable_yoast_seo() {
    _enable_wp_seo_plugin \
        ...
        --conf_file "yoast-seo-sitemap.conf" \
        --conf_remove "rank-math-sitemap.conf"
}

enable_rank_math_seo() {
    _enable_wp_seo_plugin \
        --plugin_name "Rank Math SEO" \
        --conf_file "rank-math-sitemap.conf" \
        --conf_remove "yoast-seo-sitemap.conf"
}
